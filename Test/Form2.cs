...
namespace Test
{
    public partial class Form2 : Form
    {
        ...
        private IDataAccess GetDataAccess(string ConnectionType, string ConnectionString)
        {
            try
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            bool tcpStoped = true;
             int timer = 30;
            IDataAccess DAC = GetDataAccess("SQLSERVER", @"server='localhost\mssqlserver2008';uid='c2k';password='C2K';database='FeiliksDataAdapter';multipleactiveresultsets=true");
            if (DAC == null)
            {
            ...
            do
            {
                ...
                if (dtConfigList != null && dtConfigList.Rows.Count > 0)
                {
                    string backupFolder, faliedFoder, successFolder;
                    foreach (DataRow drConfig in dtConfigList.Rows)
                    {
                        #region  数据库配置检验
                        ...
                        foreach (string fileFullName in Directory.GetFiles(drConfig["TransformFolder"].ToString().Trim()))
                        {
                            ...
                            try
                            {
                                ...
                                MoveFile(successFolder + "\\" + strOriginalFileName, drConfig["TransformSuccessFolder"].ToString().Trim());
