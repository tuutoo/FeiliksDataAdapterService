    ...
    public class FeiliksDataAdapter : System.ServiceProcess.ServiceBase
    {
        ...
        private void Service()
        {
            ...
            do
            {
                                    ...
                                    try
                                    {
                                        if (msg != null && msg.Body != null)
                                        {
                                            ...
                                            if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                            {
                                                ...
                                                {
                                                    CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文接收成功", splitFileName[1].Trim());
                                                }
                                            }
                                            #endregion
                                        }
                        ...
                        foreach (string fileFullName in Directory.GetFiles(drConfig["CheckFolder"].ToString().Trim()))
                        {
                            ...
                            if (fileInfo.Length <= 0)
                            {
                                ...
                                if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                {
                                    ...
                                    {
                                        CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文校验失败：报文无内容", splitFileName[1].Trim());
                                    }
                                }
                                continue;
                            }
                ...
                if (dtConfigList != null && dtConfigList.Rows.Count > 0)
                {
                    string backupFolder, faliedFoder;
                    foreach (DataRow drConfig in dtConfigList.Rows)
                    {
                        #region  数据库配置检验
                        ...
                        #endregion
                        backupFolder = drConfig["TransformBackupFolder"].ToString().Trim() + "\\" + strMonthDate;
                        if (!Directory.Exists(backupFolder))
                        {
                            Directory.CreateDirectory(backupFolder);
                        }
                        faliedFoder = drConfig["TransformBackupFolder"].ToString().Trim() + "\\Failed";
                        if (!Directory.Exists(faliedFoder))
                        {
                            Directory.CreateDirectory(faliedFoder);
                        }
                        ...
                        foreach (string fileFullName in Directory.GetFiles(drConfig["TransformFolder"].ToString().Trim()))
                        {
                            ...
                            try
                            {
                                ...
                                transform.Transform(fileFullName, backupFolder + "\\" + strOriginalFileName);
                                MoveFile(fileFullName, drConfig["TransformSuccessFolder"].ToString().Trim());
                                ...
                                if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                {
                                    if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                    {
                                        CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文转换成功", splitFileName[1].Trim());
                                    }
                                }
                                #endregion
                            }
                            ...
                            {
                                //此处可以生成转换错误的回执文件，暂无
                                MoveFile(fileFullName, faliedFoder);
                                this.WriteLog("报文转换异常：" + ex2.Message);
                                //记录接收历史记录
                                SaveDataAdapterHistory(DAC, drConfig["TransformSuccessFolder"].ToString().Trim(), faliedFoder + "\\" + strOriginalFileName, drConfig["Indx"].ToString().Trim(), StatusCode.Falied, "XSLT报文转换失败:" + ex2.Message, ConfigType.Transform, strOriginalFileName);
                                ...
                                if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                {
                                    if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                    {
                                        CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文转换失败:" + ex2.Message, splitFileName[1].Trim());
                                    }
                                }
                                #endregion
                                continue;
                ...
                if (dtConfigList != null && dtConfigList.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drConfig in dtConfigList.Rows)
                    {
                        ...
                        if (drConfig["SendFolder"].ToString().Trim() == "")
                        {
                            ...
                            SetConfigDisabled(DAC, drConfig["Indx"].ToString().Trim(), "FDA_TransformConfig", "报文发送的待发送路径不能为空");
                            continue;
                        }
                        if (Directory.Exists(drConfig["SendFolder"].ToString().Trim()) == false)
                        {
                            WriteLog("报文发送的待发送路径不存在。任务编号：" + drConfig["TaskCode"].ToString().Trim().ToLower());
                            SetConfigDisabled(DAC, drConfig["Indx"].ToString().Trim(), "FDA_TransformConfig", "报文发送的待发送路径不存在");
                            continue;
                        }
                        ...
                        if (drConfig["BakFolder"].ToString().Trim() == "")
                        {
                            ...
                            SetConfigDisabled(DAC, drConfig["Indx"].ToString().Trim(), "FDA_TransformConfig", "报文发送的备份路径不能为空");
                            continue;
                        }
                        if (Directory.Exists(drConfig["BakFolder"].ToString().Trim()) == false)
                        {
                            WriteLog("报文发送的备份路径不存在。任务编号：" + drConfig["TaskCode"].ToString().Trim().ToLower());
                            SetConfigDisabled(DAC, drConfig["Indx"].ToString().Trim(), "FDA_TransformConfig", "报文发送的备份路径不存在");
                            continue;
                        }
                        #endregion
                        ...
                        foreach (string fileFullName in Directory.GetFiles(drConfig["SendFolder"].ToString().Trim()))
                        {
                            ...
                            if (fileInfo.Length <= 0)
                            {
                                ...
                                if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                {
                                    if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                    {
                                        CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文发送失败:报文无内容", splitFileName[1].Trim());
                                    }
                                }
                                #endregion
                                continue;
                            ...
                            if (drConfig["SendCountLimit"].ToString().Trim() != "" && int.TryParse(drConfig["SendCountLimit"].ToString().Trim(), out intTemp1) && intTemp1 > 0)
                            {
                                ...
                                if (SendLimit != null && SendLimit.Rows.Count > 0 && SendLimit.Rows.Count > intTemp1)
                                {
                                    ...
                                    if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                    {
                                        if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                        {
                                            CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文发送失败:报文发送次数已达最大限制", splitFileName[1].Trim());
                                        }
                                    }
                                    #endregion
                                }
                            ...
                            if (drConfig["TransType"].ToString().Trim().ToLower() == "mq")
                            {
                                ...
                                try
                                {
                                    ...
                                    if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                    {
                                        if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                        {
                                            CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文发送成功", splitFileName[1].Trim());
                                        }
                                    }
                                    #endregion
                                }
                                catch (Exception ex)
                                {
                                    ...
                                    if (drConfig["IsSendReceipt"].ToString().Trim() == "1" && drConfig["ReceiptFolder"].ToString().Trim() != "" && Directory.Exists(drConfig["ReceiptFolder"].ToString().Trim()) && drConfig["TaskCode"].ToString().Trim() != "")
                                    {
                                        if (splitFileName.Length >= 2 && splitFileName[0].Trim() != "" && splitFileName[0].Trim() == drConfig["TaskCode"].ToString().Trim() && splitFileName[1].Trim() != "")
                                        {
                                            CreateReceiptFile(drConfig["ReceiptFolder"].ToString().Trim() + "\\" + splitFileName[0].Trim() + "$" + splitFileName[1].Trim() + "$Receipt$" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff") + ".xml", "报文发送失败:" + ex.Message, splitFileName[1].Trim());
                                        }
                                    }
                                    #endregion
                                }
                ...
                foreach (string arg in args)
                {
                    switch (arg.ToLower())
                    {
                        case "service":
                            ...
                            ServicesToRun = new System.ServiceProcess.ServiceBase[] { new FeiliksDataAdapter() };
                        ...
                        case "uninstall":
                            ...
                            break;
                        case "start":
                            System.ServiceProcess.ServiceController ctlstart = new ServiceController("FeiliksDataAdapter");
                            System.Console.WriteLine("Start Service Please Wait...");
                            ctlstart.Start();
                            Console.WriteLine("Service Started");
                            break;
                        case "stop":
                            System.ServiceProcess.ServiceController ctlstop = new ServiceController("FeiliksDataAdapter");
        ...
        static void Install()
        {
            try
            {
                ...
                System.IO.Stream buffer = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("InstallUtil.exe");
                ...
                System.ServiceProcess.ServiceController ctl = new ServiceController("FeiliksDataAdapter");
                System.Console.WriteLine("Start Service Please Wait...");
                ctl.Start();
                Console.WriteLine("Service Started");
            }
        ...
        private void InitializeComponent()
        {
            ...
            this.ServiceName = "FeiliksDataAdapter";
        }
        ...
        protected override void OnStart(string[] args)
        {
            ...
            {
                System.ServiceProcess.ServiceController sc = new ServiceController("FeiliksDataAdapter");
                sc.Stop();
                sc.Dispose();
            }
        }
